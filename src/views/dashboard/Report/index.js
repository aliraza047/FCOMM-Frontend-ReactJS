...
const index = () => {
    ...
    const { complete_order, cancel_order, product_report } = useSelector((state) => state._report);

    const { dashboard_data } = useSelector((state) => state._user);
    Log('dashboard_data', complete_order);
    useEffect(() => {
        dispatch(getDashboardData());
        dispatch(getAllProducts());
    }, []);

    console.log('Data Series==>', graphSeriesMethod(product_report?.stats, 'totalAmount'));
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                            <CardContent className="textContainer" sx={{ flex: '1 0 auto' }}>
                                ...
                                <Typography variant="h3" color="text.secondary" component="div">
                                    ${getGraphSeriestotal(graphSeriesMethod(dashboard_data?.netIncome, 'manufacturerIncome'))}
                                </Typography>
                            </CardContent>
                        </Box>
                        ...
                        <ReactApexChart
                            ...
                            series={
                                isArrayCheck(cancel_order?.stats)
                                    ? [{ name: 'cancel order', data: graphSeriesMethod(cancel_order?.stats, 'totalOrder') }]
                                    : series
                            }
                            type="area"
                            height={250}
